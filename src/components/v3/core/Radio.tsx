...
export function Radio<T extends string, U extends T>(
    { className, name, current, value, required, disabled, onChange }: Props<T, U>,
) {
    const [id] = React.useState(newId);

    return <span className={`relative inline-block h-6 w-6 align-bottom ${className ?? ''}`}>
        ...
        <div className={computeClassName(disabled)}>
            {value === current &&
                <div className={`rounded-full h-2.5 w-2.5 ${disabled ? 'bg-gray-400' : 'bg-primary'}`} />
            }
...
function computeClassName(disabled: boolean | undefined): string {
    ...
    let className = 'flex w-full absolute h-full items-center justify-center group cursor-pointer ' +
    ...
    if (disabled) {
        ...
        className += ' group-hover:bg-primary-light/10 hover:bg-primary-light/10 border-primary';
    }
    return className;
}
