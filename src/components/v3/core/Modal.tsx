...
export const ModalRaw: React.FC<RawProps & { willBeRemoved: boolean }> = (
    { header, children, small, className, noClose, onClose, willBeRemoved },
) => {
    ...
    return createPortal(
        <div className={`fixed flex items-center justify-center inset-0 transition-colors ${willBeRemoved ? 'bg-transparent' : 'bg-black/50'}`}
            style={{ zIndex }}
        >
            <div className={computeModalClassName(small, willBeRemoved)}>
                <div className={`w-full flex px-8 pt-8 ${header ? 'justify-between' : 'justify-end'}`}>
                    {header}
                    {!noClose && <button className="outline-none focus-visible:outline-black" onClick={onClose}>
...
function computeModalClassName(small: boolean | undefined, willBeRemoved: boolean): string {
    let className = 'w-full rounded-md shadow-2xl flex flex-col dark:bg-gray-900 dark:text-white bg-white';
