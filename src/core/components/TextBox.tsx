  ...
  const textFieldInner = (
    <div className={classes.join(" ")}>
      <div className="relative flex items-center w-full h-full cursor-text">
        ...
        />
        {prependIcon ? (
          <PrependIcon icon={prependIcon} onClick={onPrependIconClick} />
        ) : null}
        {appendIcon ? (
          <AppendIcon
            submit={appendSubmit}
...
// tslint:disable-next-line: variable-name
export const PrependIcon: FC<{ icon: string; onClick?: () => void }> = ({
  icon,
  onClick,
}) => (
  <div
    onClick={onClick}
    className="peer-focus:text-primary text-gray-300 pl-3 order-first"
  >
    <i className={icon} />
  </div>
);
...
export const AppendIcon: FC<{
  ...
  submit ? (
    <button
      type="submit"
      onClick={onClick}
      className="peer-focus:text-gray-500 text-gray-300 px-1 mr-2 outline-none focus-visible:outline-black"
    >
      <i className={icon} />
    </button>
  ) : (
    <div
      onClick={onClick}
      className="peer-focus:text-gray-500 text-gray-300 pr-3"
    >
      <i className={icon} />
    </div>
  );
