...
export function Dropdown<
  T extends string | { name: string; id: string } | number
>({ onChange, options, value, optionToNode, ...rest }: Props<T>) {
  if (!optionToNode) {
    optionToNode = (val) => {
  ...
  return (
    <DropdownBox selectedEntry={optionToNode(value, true)} {...rest}>
      {options.map((val) => (
        <button
          key={toKey(val)}
          type="button"
          onClick={() => onChange(val)}
          className="w-full focus-visible:outline-black py-2 px-4 hover:text-primary flex justify-between items-center outline-none hover:bg-primary-light/10"
        >
          {optionToNode!(val, false)}
          {val === value && (
            <i className="fas fa-check-circle text-primary relative -right-1" />
          )}
        </button>
      ))}
    </DropdownBox>
...
export const DropdownBox: FC<DropdownBoxProps> = ({
  ...
  const btnClassName =
    "rounded-md flex justify-between items-center group outline-none py-2 px-4 w-full flex-auto bg-transparent " +
    "disabled:text-gray-400 dark:text-gray-100 dark:disabled:text-gray-600 " +
    (open ? "text-primary" : "");

  let dropdownClassName =
    "absolute top-0 focus-within:ring focus-within:ring-opacity-[28%] w-full border border-solid transition-all flex flex-col items-start";
  ...
  if (invalid && !open) {
    ...
    // Focus
    dropdownClassName +=
      " dark:border-gray-700 border-gray-300 hover:border-primary focus-within:ring-primary";
  } else if (disabled) {
    ...
    <div className={className}>
      ...
      <div className="relative left-0 right-0 w-full text-sm">
        ...
        {/*
            ...
            */}
        <div className="opacity-0 py-2 px-4 border border-solid">
          {selectedEntry}
        </div>
      </div>
    </div>
