...
export const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  (props, ref) => {
    const { onClick, children, style, type, title, ...state } = props;
    ...
    return (
      <button
        ...
        style={style}
        className={computeClassName(state)}
        onClick={onClick}
        title={title}
      >
        {children}
...
// tslint:disable-next-line: variable-name
export const ExternalLinkButton: FC<ExternalLinkProps> = ({
  href,
  children,
  style,
  title,
...
function computeClassName(state: ClassProps): string {
  const colorModifier = state.disabled
