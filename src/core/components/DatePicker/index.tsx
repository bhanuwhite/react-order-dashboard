  ...
  return (
    <div className={className}>
      <div className={wrapperClassName}>
        {labelStart && (
          <label className={labelClassName} htmlFor={idStart}>
            {labelStart}
          </label>
        )}
        <div className="relative left-0 right-0 w-full text-sm">
          <div ref={refStart} className={topClassName} onBlur={onFocusChange}>
            <div
              ...
              } ${open ? "border-b dark:border-gray-700 border-gray-300" : ""}`}
            >
              <input
                readOnly
                disabled={disabled}
      ...
      <div
        className={`h-px w-5 dark:bg-gray-700 bg-gray-300 ${
          hasLabel ? "relative top-3" : ""
        }`}
      />
      <div className={wrapperClassName}>
        {labelEnd && (
          <label className={labelClassName} htmlFor={idEnd}>
            {labelEnd}
          </label>
        )}
        <div className="relative left-0 right-0 w-full text-sm">
...
function formatValue(
  ...
  if (dateOnly) {
    return (
      ...
      (time.isSame(new Date(), "day") ? " (Today)" : "")
    );
  }
  if (timeOnly) {
  ...
  return (
    time.format("MMM DD, YYYY, h:mm A") +
    (time.isSame(new Date(), "day") ? " (Today)" : "")
  );
}
...
function computeClassNames({
  ...
  topClassName +=
    " absolute top-0 w-full inline-block border border-solid rounded-md transition-all";
  ...
  const inputClassName =
    "peer text-sm rounded-md focus:outline-none py-2 px-4 w-full flex-auto dark:text-gray-100" +
  ...
  const iconClassName =
    ...
    (open
      ? "text-primary"
      : "text-gray-300 dark:text-gray-100 dark:peer-disabled:text-gray-600");
