 ...
 */
export function useOnClickOutsideManyRefs(onClickOutside: () => void, ...refs: React.RefObject<HTMLElement>[]) {

    React.useEffect(() => {

        function handleClickOutside(ev: MouseEvent) {
            // FIXME: are we sure this cast is correct?
            if (ev.target && refs.all(ref => ref.current && !ref.current.contains(ev.target as Node))) {
                onClickOutside();
            }
        }
